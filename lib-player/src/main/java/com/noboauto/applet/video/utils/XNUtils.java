package com.noboauto.applet.video.utils;
...
public class XNUtils {
    ...
    public static void setRequestedOrientation(Context context, int orientation) {
        if (XNUtils.scanForActivity(context) != null) {
            XNUtils.scanForActivity(context).setRequestedOrientation(
                    orientation);
        } else {
            XNUtils.scanForActivity(context).setRequestedOrientation(
                    orientation);
    ...
    public static Window getWindow(Context context) {
        if (XNUtils.scanForActivity(context) != null) {
            return XNUtils.scanForActivity(context).getWindow();
        } else {
            return XNUtils.scanForActivity(context).getWindow();
        }
    }
    ...
    public static long getSavedProgress(Context context, Object url) {
        if (!Xnvd.SAVE_PROGRESS) return 0;
        SharedPreferences spn = context.getSharedPreferences("JZVD_PROGRESS",
                Context.MODE_PRIVATE);
    ...
    public static void showStatusBar(Context context) {
        if (Xnvd.TOOL_BAR_EXIST) {
            XNUtils.getWindow(context).clearFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN);
        }
    }
    ...
    public static void hideStatusBar(Context context) {
        if (Xnvd.TOOL_BAR_EXIST) {
            XNUtils.getWindow(context).setFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN, WindowManager.LayoutParams.FLAG_FULLSCREEN);
        }
    }
    ...
    public static void hideSystemUI(Context context) {
        ...
        SYSTEM_UI = XNUtils.getWindow(context).getDecorView().getSystemUiVisibility();
        XNUtils.getWindow(context).getDecorView().setSystemUiVisibility(uiOptions);
    ...
    public static void showSystemUI(Context context) {
        ...
        XNUtils.getWindow(context).getDecorView().setSystemUiVisibility(SYSTEM_UI);
    }
